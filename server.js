...
app.get('/api/vectors', (req, res) => {
  res.json({ success: true, total: VECTORS.length, vectors: VECTORS });
});
...
app.get('/api/logs', (req, res) => {
  ...
  res.json({ success: true, total: logs.length, logs: logs.slice(0, parseInt(limit)) });
});
...
app.get('/api/metrics', (req, res) => {
  ...
  crashLogs.forEach(log => {
    if (!byVector[log.vector]) byVector[log.vector] = { count: 0, success: 0 };
